...
import { extractName } from './pipeline/extract/name';
  ...
  return {
    parse(text: string, context: ParseContext, parseOptions: ParseOptions): ParseResult {
      ...
      for (const seg of segments) {
        ...
        transactions.push({ ...result, source: { text: originalText, ...originalSpan } });
      }
...
function parseSegment(
  ...
  const dateScored = scoreField(dateResult.field, 'date', threshold);
